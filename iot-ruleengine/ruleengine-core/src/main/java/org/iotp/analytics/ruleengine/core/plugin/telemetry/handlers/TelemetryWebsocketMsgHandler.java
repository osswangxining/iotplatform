...
import org.iotp.analytics.ruleengine.core.plugin.telemetry.cmd.AttributesSubscriptionCmd;
import org.iotp.analytics.ruleengine.core.plugin.telemetry.cmd.GetHistoryCmd;
import org.iotp.analytics.ruleengine.core.plugin.telemetry.cmd.SubscriptionCmd;
import org.iotp.analytics.ruleengine.core.plugin.telemetry.cmd.TelemetryPluginCmd;
import org.iotp.analytics.ruleengine.core.plugin.telemetry.cmd.TelemetryPluginCmdsWrapper;
import org.iotp.analytics.ruleengine.core.plugin.telemetry.cmd.TimeseriesSubscriptionCmd;
...
public class TelemetryWebsocketMsgHandler extends DefaultWebsocketMsgHandler {
  ...
  private void handleWsAttributesSubscriptionCmd(PluginContext ctx, PluginWebsocketSessionRef sessionRef,
        ...
        if (keysOptional.isPresent()) {
          ...
          PluginCallback<List<AttributeKvEntry>> callback = new PluginCallback<List<AttributeKvEntry>>() {
            ...
            public void onSuccess(PluginContext ctx, List<AttributeKvEntry> data) {
              ...
              SubscriptionState sub = new SubscriptionState(sessionId, cmd.getCmdId(), entityId,
                  SubscriptionType.ATTRIBUTES, false, subState);
          ...
          PluginCallback<List<AttributeKvEntry>> callback = new PluginCallback<List<AttributeKvEntry>>() {
            ...
            public void onSuccess(PluginContext ctx, List<AttributeKvEntry> data) {
              ...
              SubscriptionState sub = new SubscriptionState(sessionId, cmd.getCmdId(), entityId,
                  SubscriptionType.ATTRIBUTES, true, subState);
              subscriptionManager.addLocalWsSubscription(ctx, sessionId, entityId, sub);
            }
  ...
  private void handleWsTimeseriesSubscriptionCmd(PluginContext ctx, PluginWebsocketSessionRef sessionRef,
      TimeseriesSubscriptionCmd cmd) {
    String sessionId = sessionRef.getSessionId();
    log.debug("[{}] Processing: {}", sessionId, cmd);
      ...
      if (cmd.isUnsubscribe()) {
        ...
        if (keysOptional.isPresent()) {
          ...
          if (cmd.getTimeWindow() > 0) {
            ...
            ctx.loadTimeseries(entityId, queries,
                getSubscriptionCallback(sessionRef, cmd, sessionId, entityId, startTs, keys));
          } else {
            List<String> keys = new ArrayList<>(getKeys(cmd).orElse(Collections.emptySet()));
            startTs = System.currentTimeMillis();
            log.debug("[{}] fetching latest timeseries data for keys: ({}) for device : {}", sessionId, cmd.getKeys(),
                entityId);
            ctx.loadLatestTimeseries(entityId, keys,
                getSubscriptionCallback(sessionRef, cmd, sessionId, entityId, startTs, keys));
          }
        } else {
          ctx.loadLatestTimeseries(entityId, new PluginCallback<List<TsKvEntry>>() {
            ...
            public void onSuccess(PluginContext ctx, List<TsKvEntry> data) {
              ...
              SubscriptionState sub = new SubscriptionState(sessionId, cmd.getCmdId(), entityId,
                  SubscriptionType.TIMESERIES, true, subState);
              subscriptionManager.addLocalWsSubscription(ctx, sessionId, entityId, sub);
            }
  ...
  private PluginCallback<List<TsKvEntry>> getSubscriptionCallback(final PluginWebsocketSessionRef sessionRef,
      final TimeseriesSubscriptionCmd cmd, final String sessionId, final EntityId entityId, final long startTs,
    ...
    return new PluginCallback<List<TsKvEntry>>() {
      ...
      public void onSuccess(PluginContext ctx, List<TsKvEntry> data) {
        ...
        data.forEach(v -> subState.put(v.getKey(), v.getTs()));
        SubscriptionState sub = new SubscriptionState(sessionId, cmd.getCmdId(), entityId, SubscriptionType.TIMESERIES,
            false, subState);
        subscriptionManager.addLocalWsSubscription(ctx, sessionId, entityId, sub);
      }
  ...
  private void handleWsHistoryCmd(PluginContext ctx, PluginWebsocketSessionRef sessionRef, GetHistoryCmd cmd) {
    ...
    List<TsKvQuery> queries = keys.stream().map(key -> new BaseTsKvQuery(key, cmd.getStartTs(), cmd.getEndTs(),
        cmd.getInterval(), getLimit(cmd.getLimit()), getAggregation(cmd.getAgg()))).collect(Collectors.toList());
  ...
  private boolean validateSessionMetadata(PluginContext ctx, PluginWebsocketSessionRef sessionRef, SubscriptionCmd cmd,
      String sessionId) {
    WsSessionMetaData sessionMD = wsSessionsMap.get(sessionId);
    if (sessionMD == null) {
      log.warn("[{}] Session meta data not found. ", sessionId);
  ...
  private boolean validateSubscriptionCmd(PluginContext ctx, PluginWebsocketSessionRef sessionRef,
      SubscriptionCmd cmd) {
    if (cmd.getEntityId() == null || cmd.getEntityId().isEmpty()) {
      SubscriptionUpdate update = new SubscriptionUpdate(cmd.getCmdId(), SubscriptionErrorCode.BAD_REQUEST,
          "Device id is empty!");
