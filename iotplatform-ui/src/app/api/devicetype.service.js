...
export default angular.module('thingsboard.api.devicetype', [thingsboardTypes])
    .factory('deviceTypeService', DeviceTypeService)
    .name;


/*@ngInject*/
function DeviceTypeService($http, $q, attributeService, customerService, types) {
    ...
    function getTenantDevices(pageLink, applyCustomersInfo, config, type) {
        ...
        var url = '/api/tenant/devicetypes?limit=' + pageLink.limit;
    ...
    function getCustomerDevices(customerId, pageLink, applyCustomersInfo, config, type) {
        var deferred = $q.defer();
        var url = '/api/customer/' + customerId + '/devicetypes?limit=' + pageLink.limit;
        if (angular.isDefined(pageLink.textSearch)) {
            url += '&textSearch=' + pageLink.textSearch;
        }
        if (angular.isDefined(pageLink.idOffset)) {
    ...
    function getDevice(deviceId, ignoreErrors, config) {
        var deferred = $q.defer();
        var url = '/api/devicetype/' + deviceId;
        if (!config) {
            config = {};
        }
    ...
    function getDevices(deviceIds, config) {
        ...
        var url = '/api/devicetypes?devicetypeIds=' + ids;
        $http.get(url, config).then(function success(response) {
    ...
    function assignDeviceToCustomer(customerId, deviceId) {
        var deferred = $q.defer();
        var url = '/api/customer/' + customerId + '/devicetype/' + deviceId;
    ...
    function unassignDeviceFromCustomer(deviceId) {
        var deferred = $q.defer();
        var url = '/api/customer/devicetype/' + deviceId;
    ...
    function makeDevicePublic(deviceId) {
        var deferred = $q.defer();
        var url = '/api/customer/public/devicetype/' + deviceId;
        $http.post(url, null).then(function success(response) {
            deferred.resolve(response.data);
        }, function fail() {
            deferred.reject();
